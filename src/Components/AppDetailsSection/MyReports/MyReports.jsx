...
const MyReports = () => {
  ...
  const handleDelete = async (_, indexChoosed) => {
    ...
    const LostItemsAfterDelete = Reports.filter(
      (_, index) => index !== indexChoosed
    );
  ...
  const handleFoundDelete = async (_, indexChoosed) => {
    const FoundItemsAfterDelete = Lostreports.filter(
      (_, index) => index !== indexChoosed
    );
    // console.log(FoundItemsAfterDelete)
...
//      {Reports.length > 0 ? <div>{Reports.map((x, index) => (
//         <div key={index} className="report-card">
//           <h4 className="item-name">{x.itemname}</h4>
//           <p><strong>Brand:</strong> {x.brand}</p>
//           <p><strong>Category:</strong> {x.category}</p>
...
//           </div>
//         </div>
//       ))}</div>:<h1>You Havent posted Lost Items Yet</h1>}
...
//         // </div>
//       ))}
//       </div>:<h1>You havent Post Found Items</h1>}
//     {/* </div> */}
