// // import React from 'react'
// // import { Button } from 'react-bootstrap';
// // const FoundItemSection = ({setCatSection}) => {
// //     const HandletwoCategory = (CatValue2) => {
// //     // setOpen(false);
// //     setCatSection(CatValue2)
// //     // setToggle(true)
// //   };
// //   return (

// // <div className="scroll-category-bar">
// //   <h5 style={{ marginBottom: '10px' }}></h5>
// //   <div className="scroll-category-buttons">
// //     {/* Buttons here */}
// //     <div style={{ padding: "10%",backgroundColor: "white", borderRadius: "10px" }} id="BtnSec2" className="sidebar-select">
// //           <h5>Found Items Category</h5>
// //           <Button onClick={() => HandletwoCategory("electronics")}>Electronics</Button>
// //           <Button onClick={() => HandletwoCategory("documents")}>Documents</Button>
// //                     <Button onClick={() => HandletwoCategory("Mobiles")}>Mobiles</Button>
// //                               <Button onClick={() => HandletwoCategory("Laptops")}>Laptops</Button>
// //                                         <Button onClick={() => HandletwoCategory("Bags")}>Bags</Button>
// //           <Button onClick={() => HandletwoCategory("wallets")}>wallets</Button>
// //         <Button onClick={() => HandletwoCategory("jewellery")}>jewellery</Button>
// //                   <Button onClick={() => HandletwoCategory("earbuds")}>EarBuds</Button>
// //           <Button onClick={() => HandletwoCategory("Books")}>Documents</Button>

// //           <Button onClick={() => HandletwoCategory("accessories")}>Accessories</Button>
// //           <Button onClick={() => HandletwoCategory("otherItems")}>OtherItems</Button>
// //           <Button onClick={() => HandletwoCategory("All")}>All</Button>
// //         </div>
// //     <div>

// //     </div>
// //   </div>
// // </div>

// //   )
// // }

// // export default FoundItemSection
// import React from 'react';
// import { Button } from 'react-bootstrap';
// import './FoundItemSection.css'; // Make sure this CSS file exists

// const FoundItemSection = ({ setCatSection }) => {
//   const HandletwoCategory = (CatValue2) => {
//     setCatSection(CatValue2);
//   };

//   return (
//     <div className="category-scroll-wrapper">
//       <h5 className="mb-3">📂 Found Items Category</h5>
//       <div className="category-scroll-buttons">
//         <Button onClick={() => HandletwoCategory("electronics")}>Electronics</Button>
//         <Button onClick={() => HandletwoCategory("documents")}>Documents</Button>
//         <Button onClick={() => HandletwoCategory("Mobiles")}>Mobiles</Button>
//         <Button onClick={() => HandletwoCategory("Laptops")}>Laptops</Button>
//         <Button onClick={() => HandletwoCategory("Bags")}>Bags</Button>
//         <Button onClick={() => HandletwoCategory("wallets")}>Wallets</Button>
//         <Button onClick={() => HandletwoCategory("jewellery")}>Jewellery</Button>
//         <Button onClick={() => HandletwoCategory("earbuds")}>EarBuds</Button>
//         <Button onClick={() => HandletwoCategory("Books")}>Books</Button>
//         <Button onClick={() => HandletwoCategory("accessories")}>Accessories</Button>
//         <Button onClick={() => HandletwoCategory("otherItems")}>Other Items</Button>
//         <Button onClick={() => HandletwoCategory("All")}>All</Button>
//       </div>
//     </div>
//   );
// };

// export default FoundItemSection;
import React from "react";
import { Button } from "react-bootstrap";
import "./FoundItemSection.css"; // This will hold scroll CSS

const FoundItemSection = ({ setCatSection }) => {
  const HandletwoCategory = (CatValue2) => {
    setCatSection(CatValue2);
  };

  return (
    <div className="scroll-container" style={{ marginTop: "4%" }}>
       <h5 className="scroll-title"> lost-Items-Category</h5>
      <div className="scroll-buttons" style={{ display: "flex" }}>
        <div>




          
          <Button onClick={() => HandletwoCategory("electronics")}>
                      <img src="https://img.freepik.com/free-photo/elegant-smartphone-composition_23-2149437105.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

         
          </Button>


          <p>
            <span id="catText">Mobile</span>
          </p>
        </div>

        <div>
          <Button onClick={() => HandletwoCategory("documents")}>
                        <img src="https://img.freepik.com/premium-vector/document-icon-paper-documents-folder-with-signature-text_399089-266.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">Documents</span>
          </p>
        </div>

        {/* <Button onClick={() => HandletwoCategory("Mobiles")}>Mobiles</Button> */}
        <div>
          <Button onClick={() => HandletwoCategory("Bags")}>
                       <img src="https://img.freepik.com/free-photo/beautiful-elegance-luxury-fashion-green-handbag_1203-7655.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">Bags</span>
          </p>
        </div>
        <div>
          <Button onClick={() => HandletwoCategory("wallets")}>
                        <img src="https://img.freepik.com/free-photo/still-life-assortment-with-cryptocurrency_23-2149102066.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">wallets</span>
          </p>
        </div>

 <div>
          <Button onClick={() => HandletwoCategory("laptops")}>
                       <img src="https://img.freepik.com/free-vector/laptop-concept-illustration_114360-24409.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">Laptops</span>
          </p>
        </div>
        <div>
          <Button onClick={() => HandletwoCategory("jewellery")}>
                       <img src="https://img.freepik.com/free-vector/jewelry-accessories-realistic-set_1284-16770.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">jewellery</span>
          </p>
        </div>

        <div>
          <Button onClick={() => HandletwoCategory("earbuds")}>
                      <img src="https://img.freepik.com/free-vector/headphones-realistic-set_1284-26322.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">earbuds</span>
          </p>
        </div>
        <div>
          <Button onClick={() => HandletwoCategory("Books")}>
                        <img src="https://img.freepik.com/free-vector/hand-drawn-flat-design-stack-books-illustration_23-2149341898.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">Books</span>
          </p>
        </div>
        <div>
          <Button onClick={() => HandletwoCategory("accessories")}>
                       <img src="https://img.freepik.com/free-photo/still-life-teenager-s-desk_23-2149371274.jpg?ga=GA1.1.1112550859.1741454000&semt=ais_hybrid&w=740" alt="" />

          </Button>
          <p>
            <span id="catText">accessories</span>
          </p>
        </div>
        <div>
          <Button onClick={() => HandletwoCategory("otherItems")}>
                        <img src="data:image/png;base64," alt="" />

          </Button>
          <p>
            <span id="catText">Other-Items</span>
          </p>
        </div>
        <div>
          <Button id="All" onClick={() => HandletwoCategory("All")}>
            All
          </Button>
          <p>
            <span id="catText">AllItems</span>
          </p>
        </div>
      </div>
   </div>
  );
};

export default FoundItemSection;
